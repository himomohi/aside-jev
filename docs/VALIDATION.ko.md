...
검증일: 2026-09-20. 소스 프리뷰 0.2.0의 로컬 검사와 GitHub 호스팅 운영체제 검사를 아래에서 구분합니다. 소스 게시는 패키지 릴리즈를 뜻하지 않습니다.
...
- 소스 `625898b`의 최종 [GitHub 호스팅 검증](https://github.com/himomohi/aside-jev/actions/runs/35501878103): **Windows Python 222개 통과, macOS 221개 통과와 Windows 전용 1개 미실행, 양쪽 JavaScript 각각 20개 통과**. wheel 빌드와 로컬 문서 링크 115개도 통과했습니다. 양쪽 모두 Python 3.11.9로 초기 설치하고 확장 자산 9개, 영문 설치, 한국어 재설치, 기존 설정 보존, 기본 OFF, Native 프레임 응답을 확인했습니다. Windows는 격리된 HKCU 등록과 정리도 확인했으며 양쪽 임시 파일을 제거했습니다.
- Windows CI에서 JSON 이스케이프 경로 비교, 실행 경로의 `%` 확장, `Get-FileHash` 명령 미발견, isolated Python의 UTF-8 출력 문제가 드러났습니다. 경로 객체 비교·미지원 실행 경로 사전 거부·내장 .NET 해시 및 ZIP API·명시적인 UTF-8 설정으로 수정했으며 최종 실행에서 해당 경로를 검증했습니다.
...
- Windows 회귀 검사에서 실제 Win32 파일 핸들·잠금과 한글·공백·`! & ^ ()` 경로의 Native `.cmd` 실행을 확인했습니다. `%`는 wrapper 진입 전에 Windows가 확장하므로 해당 실행 경로는 쓰기 전에 거부합니다. 레지스트리 소유권·뷰 충돌과 실패 복구는 fake registry를 사용합니다. 임의의 Windows ACL 구성과 실제 Aside 브라우저 등록은 이 검사로 입증하지 않습니다.
